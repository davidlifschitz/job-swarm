...
| Product gate CI | Done | `plans/2026-06-29-job-swarm-completion-orchestration.md` | `.github/workflows/ci.yml` `product-gates` job runs `./scripts/verify-ops-readiness.sh` on PR/push (PR #5 merged): product gate tests, parity, load, offline seed audit, and cutover dry-run. | 2026-06-30 |
| Nightly seed audit (scheduled) | Done | `plans/2026-06-29-job-swarm-completion-orchestration.md` | Offline `scripts/seed_refresh_audit.py` + `tests/test_seed_refresh_audit.py` pass locally; `.github/workflows/nightly-seed-audit.yml` on main (06:00 UTC cron + `workflow_dispatch`). | 2026-06-30 |
