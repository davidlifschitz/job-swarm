...
# Table order respects foreign-key dependencies (llm_requests before fit/rewrite).
...
def apply_postgres_schema(db: PostgresDatabase) -> None:
    statements = [
        statement.strip()
...
def list_postgres_tables(db: PostgresDatabase) -> set[str]:
    rows = db.execute(
        """
        SELECT table_name
...
def postgres_table_columns(db: PostgresDatabase, table: str) -> set[str]:
